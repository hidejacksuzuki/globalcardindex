...
async function handle(req: NextRequest) {
  ...
  if (!result) {
    ...
    await writeCronLog("daily-discord", "error", {
  ...
  const ua2 = req.headers.get("user-agent") ?? "";
  const tBy = ua2.includes("vercel-cron") ? "cron" as const : "manual" as const;
  await writeCronLog("daily-discord", "ok", {
    durationMs, triggeredBy: tBy,
    result: { date: targetDate, messageId: result.messageId, fieldCount } as Record<string, unknown>,
  });
