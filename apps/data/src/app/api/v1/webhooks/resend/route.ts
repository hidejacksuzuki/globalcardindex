...
// ── Privacy helper: mask email address in logs ────────────────────────────────
...
function verifySvixSignature(
  ...
  // 定数時間比較（タイミング攻撃対策）: Node.js の crypto.timingSafeEqual を使用
  const expectedBuf = Buffer.from(expectedSig);
  const matches = signatures.some((sig) => {
    try {
      const sigBuf = Buffer.from(sig);
      // timingSafeEqual はバッファ長が一致しないと例外を投げる
      if (sigBuf.length !== expectedBuf.length) return false;
      return cryptoTimingSafeEqual(sigBuf, expectedBuf);
    } catch {
      return false;
    }
  });
  ...
  switch (eventType) {
    ...
    case "email.bounced": {
      ...
      for (const email of emails) {
        ...
        if (!result.ok) {
          errorCount++;
          console.error(`[webhook/resend] bounce error for ${maskEmail(email)}:`, result.error);
        } else if (result.action === "bounced") {
          processedCount++;
          console.log(`[webhook/resend] bounced: ${maskEmail(email)} (type=${bounceType})`);
        } else {
          skippedCount++;
          console.log(`[webhook/resend] bounce skipped: ${maskEmail(email)}`);
        }
      }
    ...
    case "email.complained": {
      ...
      for (const email of emails) {
        ...
        if (!result.ok) {
          errorCount++;
          console.error(`[webhook/resend] complaint error for ${maskEmail(email)}:`, result.error);
        } else if (result.action === "unsubscribed") {
          processedCount++;
          console.log(`[webhook/resend] complaint→unsubscribed: ${maskEmail(email)}`);
        } else {
          skippedCount++;
          console.log(`[webhook/resend] complaint skipped: ${maskEmail(email)}`);
        }
      }
