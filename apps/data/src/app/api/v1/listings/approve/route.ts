 ...
 *
 * Approves or rejects RawListings, then creates a PriceSnapshot
 * from the approved set for the card.
 *
 * Body:
 * {
 *   ids:    string[],   // RawListing IDs to approve
 *   reject: string[],   // RawListing IDs to reject (optional)
 * }
 *
 * Response:
 * { ok, approved, rejected, snapshot: PriceSnapshot | null }
 */
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const [approveResult, rejectResult] = await Promise.all([
    approveIds.length > 0
      ? prisma.rawListing.updateMany({ where: { id: { in: approveIds } }, data: { status: "approved" } })
      : Promise.resolve({ count: 0 }),
    rejectIds.length > 0
      ? prisma.rawListing.updateMany({ where: { id: { in: rejectIds } }, data: { status: "rejected" } })
      : Promise.resolve({ count: 0 }),
  ]);

  // Determine cardId from approved listings
  let snapshot = null;
  if (approveIds.length > 0) {
    const sample = await prisma.rawListing.findFirst({
    ...
    if (sample) {
      ...
      if (approved.length > 0) {
        ...
        const trimmed     = sortedP.slice(0, Math.ceil(sortedP.length * 0.9));
        ...
        snapshot = await prisma.priceSnapshot.create({
          data: {
            ...
            avgPrice,
            maxPrice:     sortedP[sortedP.length - 1],
