...
export default function BookmarkletPage() {
  ...
  // ── ヤフオク落札済み ブックマークレット ──────────────────────────────────────
  const jsYahoo = `javascript:(function(){if(document.getElementById('gci-yah'))return;var API='${apiUrl}',SEC='${cronSecret}';function scrape(){var items=[],seen=new Set();var rows=[].slice.call(document.querySelectorAll('li.Product,li[class*="product"],li[class*="Item"]'));if(!rows.length)rows=[].slice.call(document.querySelectorAll('li')).filter(function(li){return li.querySelector('a[href*="auctions.yahoo"]');});rows.forEach(function(li){var link=li.querySelector('a[href*="auctions.yahoo.co.jp/auction/"],a[href*="page.auctions.yahoo.co.jp"]');if(!link)return;var url=link.href.split('?')[0];if(seen.has(url))return;seen.add(url);var titleEl=li.querySelector('h3 a,[class*="title"] a,[class*="Title"] a');var title=titleEl?titleEl.textContent.trim():link.textContent.trim();if(!title||title.length<3)return;var text=li.textContent;var pm=text.match(/([0-9,]{3,})円/);if(!pm)return;var price=parseInt(pm[1].replace(/,/g,''),10);if(price<100||price>100000000)return;var bidM=text.match(/([0-9]+)件/);var bidCount=bidM?parseInt(bidM[1],10):0;items.push({title:title,price:price,url:url,bidCount:bidCount});});return items;}async function fetchCards(){try{var r=await fetch(API+'/api/v1/cards?limit=500',{headers:{'Authorization':'Bearer '+SEC}});var d=await r.json();return d.cards||[];}catch(e){return[];}}async function init(){var panel=document.createElement('div');panel.id='gci-yah';panel.style.cssText='position:fixed;bottom:20px;right:20px;z-index:999999;background:#fff;border:1px solid #ddd;border-radius:10px;box-shadow:0 4px 24px rgba(0,0,0,.15);padding:16px;width:320px;font-family:-apple-system,sans-serif;font-size:13px;color:#1a1a2e;';panel.innerHTML='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;"><span style="font-weight:600;">GCI (ヤフオク落札)</span><button id="gci-yx" style="background:none;border:none;cursor:pointer;color:#999;font-size:16px;">✕</button></div><div style="margin-bottom:6px;"><input id="gci-yf" type="text" placeholder="カード名で絞り込み..." style="width:100%;box-sizing:border-box;padding:6px 8px;border:1px solid #ddd;border-radius:6px;font-size:12px;outline:none;"></div><div style="margin-bottom:10px;"><select id="gci-ysel" style="width:100%;padding:6px 8px;border:1px solid #ddd;border-radius:6px;font-size:12px;"><option value="">読み込み中...</option></select></div><div id="gci-ycnt" style="font-size:12px;color:#666;margin-bottom:12px;"></div><button id="gci-ybtn" style="width:100%;padding:9px;background:#6d28d9;color:#fff;border:none;border-radius:6px;font-size:13px;font-weight:500;cursor:pointer;">取り込む</button><div id="gci-ymsg" style="margin-top:10px;font-size:12px;text-align:center;min-height:16px;"></div>';document.body.appendChild(panel);document.getElementById('gci-yx').onclick=function(){panel.remove();};var items=scrape();document.getElementById('gci-ycnt').textContent='検出: '+items.length+'件';var allCards=[];var cards=await fetchCards();var sel=document.getElementById('gci-ysel');function renderOptions(q){var filtered=q?allCards.filter(function(c){return(c.name+' '+c.rarity+' '+c.setName).toLowerCase().indexOf(q.toLowerCase())>=0;}):allCards;sel.innerHTML='<option value="">'+(q?filtered.length+'件絞り込み中':'カードを選択...')+'</option>'+filtered.map(function(c){return'<option value="'+c.id+'">'+c.name+' '+c.rarity+' '+c.setName+'</option>';}).join('');}if(cards.length>0){allCards=cards;renderOptions('');}else{sel.innerHTML='<option value="">カード取得失敗</option>';}document.getElementById('gci-yf').oninput=function(){renderOptions(this.value);};document.getElementById('gci-ybtn').onclick=async function(){var cardId=sel.value;if(!cardId){ymsg('カードを選択してください',1);return;}if(!items.length){ymsg('データが見つかりませんでした',1);return;}var btn=document.getElementById('gci-ybtn');btn.disabled=true;btn.textContent='送信中...';ymsg('');try{var r=await fetch(API+'/api/v1/import/yahoo-auction',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+SEC},body:JSON.stringify({cardId:cardId,mode:'closed',items:items})});var d=await r.json();if(d.ok){ymsg('✓ '+d.saved+'件取り込みました（スキップ: '+d.skipped+'）');}else{ymsg('エラー: '+d.error,1);}}catch(e){ymsg('通信エラー: '+e.message,1);}finally{btn.disabled=false;btn.textContent='取り込む';}};function ymsg(t,e){var el=document.getElementById('gci-ymsg');el.textContent=t;el.style.color=e?'#e74c3c':'#27ae60';}}init();})();`;

  return (
    <div className="space-y-12 max-w-2xl">
      <header>
        <h1 className="text-xl font-semibold text-navy">ブックマークレット</h1>
        <p className="mt-1 text-sm text-navy/60">
          Mercari・ヤフオクの検索ページで起動して価格データを取り込みます。
        </p>
      </header>

      {/* ── Mercari ─────────────────────────────────────────────── */}
      <div className="space-y-6">
        <h2 className="text-base font-semibold text-navy border-b border-navy/10 pb-2">Mercari</h2>
      ...
      {/* ── ヤフオク落札済み ──────────────────────────────────────── */}
      <div className="space-y-6">
        <h2 className="text-base font-semibold text-navy border-b border-navy/10 pb-2">ヤフオク落札済み</h2>
        ...
        <div className="rounded border border-navy/10 bg-white p-6 space-y-4">
          ...
          <ol className="space-y-2 text-xs text-navy/70">
            <li className="flex gap-2"><span className="font-bold text-navy shrink-0">1.</span>下のコードをクリックして全選択 → コピー（⌘+A → ⌘+C）</li>
            <li className="flex gap-2"><span className="font-bold text-navy shrink-0">2.</span>ブックマークバーを右クリック →「ページを追加...」</li>
            <li className="flex gap-2"><span className="font-bold text-navy shrink-0">3.</span>名前「GCI ヤフオク」、URLに貼り付けて保存</li>
          </ol>
          <BookmarkletCode code={jsYahoo} />
        </div>
        ...
        <div className="rounded border border-navy/10 bg-white p-6 space-y-3">
          ...
          <ol className="space-y-2 text-sm text-navy/70">
            <li className="flex gap-2">
              <span className="shrink-0 font-bold text-navy">1.</span>
              <span>ヤフオクの<strong>落札相場</strong>検索ページを開く（closedsearch）</span>
            </li>
            <li className="flex gap-2">
              <span className="shrink-0 font-bold text-navy">2.</span>
              <span>ブックマークバーの「GCI ヤフオク」をクリック</span>
            </li>
            <li className="flex gap-2">
              <span className="shrink-0 font-bold text-navy">3.</span>
              <span>検索ボックスでカードを絞り込み → 選択 → 「取り込む」</span>
            </li>
            <li className="flex gap-2">
              <span className="shrink-0 font-bold text-navy">4.</span>
              <span>gci-data の <strong>Cards → Collect → ヤフオク落札済み</strong> で承認</span>
            </li>
          </ol>
        </div>
      </div>{/* end ヤフオク */}
