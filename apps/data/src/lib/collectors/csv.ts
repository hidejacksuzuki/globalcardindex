...
import { clampTrustScore, DEFAULT_TRUST } from "@gci/core";
import { attachFingerprint } from "@gci/core";
import { computeTrustScore } from "@gci/core";
import { normalizeListingType } from "@gci/core";
...
export type ImportCsvOptions = {
  /** dryRun=true: parse + validate only, no DB writes */
  dryRun?: boolean;
};
...
export async function importCsv(
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    try {
      ...
      // ── Source upsert（dryRun: skip DB write, use stub）──
      const source = dryRun
        ? { id: "dry-source", name: sourceName, type: sourceType, defaultTrustScore: DEFAULT_TRUST, trustWeight: 1.0 }
      ...
      // ── Card upsert（dryRun: skip DB write, use stub）──
      const card = dryRun
        ? { id: "dry-card" }
      ...
      // ── Insert（dryRun: skip DB write, count as imported）──
      if (dryRun) {
        summary.imported++;
      } else {
