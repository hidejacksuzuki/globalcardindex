 ...
 *
 * Daily Market Recap 用 OG 画像。
 * 日付・指数変動・Top Gainer / Loser を1枚に凝縮。
 */

import { ImageResponse }  from "next/og";
import { getDailyRecap }  from "@/actions/recap";
...
export default async function Image() {
  ...
  const displayDate = new Date(recap.date).toLocaleDateString("ja-JP", {
    ...
    >
      ...
      <div
        style={{
          ...
          fontSize: 200, fontWeight: 800, color: "rgba(255,255,255,0.025)",
          letterSpacing: -6, lineHeight: 1, display: "flex",
        }}
      >
        DAILY
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        ...
        >
          ...
          <span style={{ fontSize: 28, fontWeight: 700, color: indexColor, marginTop: 4, display: "flex" }}>
            {indexStr}
          </span>
          {recap.index && (
            <span style={{ fontSize: 13, color: MUTED, display: "flex" }}>
              {recap.index.value.toFixed(2)}
            </span>
          )}
        </div>
      </div>

      {/* ── 3カラム: Gainer / Loser / Spike ────────────────── */}
      <div
        style={{
          display: "flex",
          gap: 16,
      ...
      >
        ...
        >
          <span style={{ fontSize: 11, letterSpacing: 2, textTransform: "uppercase", color: GOLD }}>
            ▲ Top Gainer
          </span>
          {topGainer ? (
            <>
              <span
                style={{
                  fontSize: topGainer.cardName.length > 18 ? 17 : 20,
        ...
        >
          <span style={{ fontSize: 11, letterSpacing: 2, textTransform: "uppercase", color: RED }}>
            ▼ Top Loser
          </span>
          {topLoser ? (
            <>
              <span
                style={{
                  fontSize: topLoser.cardName.length > 18 ? 17 : 20,
                  fontWeight: 700, color: WHITE, marginTop: 8,
                  lineHeight: 1.2, display: "flex",
                }}
              >
        ...
        >
          ...
          {topSpike ? (
            <>
              <span
                style={{
                  fontSize: topSpike.cardName.length > 18 ? 17 : 20,
                  fontWeight: 700, color: WHITE, marginTop: 8,
                  lineHeight: 1.2, display: "flex",
                }}
              >
              ...
              <span
                style={{
                  fontSize: 26, fontWeight: 700, marginTop: 8, display: "flex",
                  color: "rgba(167,139,250,0.9)",
                }}
              >
                {topSpike.count24h} <span style={{ fontSize: 14, marginLeft: 4, marginTop: 8, fontWeight: 400, color: MUTED }}>listings</span>
              </span>
            </>
          ) : (
            <span style={{ fontSize: 14, color: MUTED, marginTop: 8, display: "flex" }}>—</span>
