...
export async function generateMetadata({ params }: { params: { locale: Locale } }): Promise<Metadata> {
  ...
  return {
    ...
    description: isEn
      ? 'Trading cards with the biggest 7-day price drops — Pokémon, One Piece, Yu-Gi-Oh!, MTG.'
      : '7日間で最も値下がりしたトレカランキング。ポケカ・ワンピース・遊戯王の暴落カードをリアルタイム追跡。',
  };
}
...
export default async function LosersPage({ params }: { params: { locale: Locale } }) {
  ...
  const statLabels = isEn
    ...
    : { top: '1位の下落率', count: 'ランクイン数', avg: '平均下落率' };
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-semibold text-navy">{t.losers.title}</h1>
              <span className="rounded-sm border border-red-200 bg-red-50 px-2 py-0.5 text-[10px] uppercase tracking-widest text-red-600">
                7d ▼
              </span>
            </div>
            <p className="mt-1 text-sm text-navy/50">{t.losers.description}</p>
          </div>
